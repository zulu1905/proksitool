...
func lockNetwork(sections []RoutineSpawner, infoAddr *string) {
...
var wireCancel context.CancelFunc

func Run() {
    s := make(chan os.Signal, 1)
    signal.Notify(s, syscall.SIGINT, syscall.SIGQUIT)
    ctx, cancel := context.WithCancel(context.Background())
    wireCancel = cancel   // global değişkene atıyoruz
    ...
    tun, err := StartWireguard(conf, logLevel)
    if err != nil {
        log.Fatal(err)
    }
